/* === Quản lý hiển thị khối ngân hàng theo phương thức thanh toán === */
const paymentSelect = document.getElementById("paymentMethod");
const bankInfo = document.getElementById("bankInfo");
const bankSelect = document.getElementById("bankSelect");
const bankDetails = document.getElementById("bankDetails");

// Ẩn khối ngân hàng khi vào trang
...
/* === Load thông tin user đăng nhập (nếu có) === */
...
function renderOrders() {
  ...
  ordersList.innerHTML = orders.map((o, i) => `
    <div style='border:1px solid #ddd;padding:8px;margin:8px 0;border-radius:6px'>
      ...
      <ul>${o.items.map(it =>
        `<li>${it.name} - SL: ${it.qty} - ${(it.price * it.qty).toLocaleString('vi-VN')}₫</li>`
      ).join("")}</ul>
