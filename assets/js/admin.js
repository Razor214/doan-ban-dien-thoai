...
//CHECK LỖI
// ========== FILE 1 SỬA: Cho phép 2-3 chữ số thay vì chỉ 2 ==========
...
function openPriceModal(mode, btn) {
    form.reset();
    priceInput.value = "";
    // ========== FILE 1 THÊM: Gọi hàm xóa viền đỏ ==========
    clearValidationStyles();
    priceModal.style.display = "flex";
    editingRow = null;

    // ========== FILE 1 THÊM: Khởi tạo và điền dropdown ==========
    allBrands = [];
    allProducts = [];
    ...
    if (mode === "edit" && btn) {
        ...
        // ========== FILE 1 SỬA: Lấy giá trị từ các cột ==========
        ...
        // ========== FILE 1 THÊM: Xử lý dropdown thương hiệu và sản phẩm ==========
        ...
        idPrice.value = idPriceValue;
        idCatePrice.value = brandId;
        idProPrice.value = productId;
        idImPrice.value = importId;
        ...
        // ========== FILE 1 THÊM: Xóa giá trị các input ẩn ==========
        idCatePrice.value = '';
        idProPrice.value = '';
        autoFillNewCodes();
    }
...
// ========== FILE 1 THÊM: Biến đệm và hàm liên kết sản phẩm và loại sản phẩm ==========
...
// ========== FILE 1 SỬA: Xóa phần gán mã cứng, sử dụng dropdown động ==========
...
form.onsubmit = (e) => {
    ...
    // ========== FILE 1 SỬA: Cập nhật thông báo lỗi ==========
    ...
    if (editingRow) {
        ...
        // ========== FILE 1 THÊM: Kiểm tra trùng mã trước khi thêm ==========
...
function searchProduct() {
    ...
    // ========== FILE 1 SỬA: Logic tìm kiếm đơn giản hơn ==========
...
function displayTon(data) {
    ...
    data.forEach((item) => {
        ...
        row.innerHTML = `
            ...
            // ========== FILE 1 SỬA: Đổi slXuat thành slBan ==========
...
// ========== FILE 1 SỬA HOÀN TOÀN: Hàm cập nhật tồn kho từ phiếu nhập ==========
/**
 ================================
...
// ========== FILE 1 SỬA: Thay processInventoryUpdate bằng updateInventoryFromImport ==========
...
// ========== FILE 1 SỬA: Sử dụng key "userList" thay vì "ListUser" ==========
    ...
    rows2.forEach((row2) => {
        ...
        resetBtn.addEventListener("click", () => {
            if (confirm("Bạn có muốn đặt lại mật khẩu?")) {
                // ========== FILE 1 SỬA: Đổi "pass" thành "password" ==========
...
function saveAndRender2() {
    // ========== FILE 1 SỬA: Lưu vào "userList" thay vì "ListUser" ==========
...
function openImportModal(mode, btn) {
    ...
    if (mode === "add") {
        ...
        // ========== FILE 1 SỬA: Sử dụng status tiếng Anh ==========
...
importForm?.addEventListener("submit", (e) => {
    ...
    // ========== FILE 1 THÊM: Lưu dữ liệu cũ để cập nhật tồn kho ==========
    let oldImport = null;
    ...
    // ========== FILE 1 SỬA: Gọi hàm cập nhật tồn kho mới ==========
...
function deleteImport(btn) {
    ...
    // ========== FILE 1 THÊM: Xử lý cập nhật tồn kho khi xóa ==========
